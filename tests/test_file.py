...
def test_from_directory(test_data_dir):
    ...
    for filename, file in file_inp.program_args.files.items():
...
def test_to_directory(test_data_dir, tmp_path):
    ...
    file_inp.program_args.write_files(tmp_path)
    for filename in file_inp.program_args.files:
...
def test_to_from_file_with_binary_data(test_data_dir, tmp_path):
    ...
    assert isinstance(file_inp.program_args.files["c0"], bytes)
    ...
    assert data["program_args"]["files"]["c0"].startswith("base64:")
